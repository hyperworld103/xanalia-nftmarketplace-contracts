...
contract BlindBox is NonGenerativeBB {
    ...
    */
    function StartSeries(
       ...
       uint256[] memory integerData, //[startTime, endTime, maxBoxes, perBoxNftMint, perBoxPrice, baseCurrency, userLimit, registrationFee]
       uint256[] memory allowedCurrencies,
        ...
        if(isGenerative){
            ...
            _registrationFee[nonGenerativeSeriesId.current()] = integerData[7];
    ...
    function setRegistrationFee(uint256 fee, uint256 seriesId) onlyOwner public {
        _registrationFee[seriesId] = fee;
    }
     function getRegistrationFee( uint256 seriesId) onlyOwner public view returns(uint256) {
        return _registrationFee[seriesId];
    }
    ...
    */
    function openBox(uint256 boxId, bool isGenerative, address collection, string memory ownerId) public {
        if(isGenerative){
            // openGenBox(boxId);
        } else {
            openNonGenBox(boxId, collection, ownerId);
        }
    }
